...
function encodeToRot13(str) {
  ...
  function getNextCharCode(charCode) {
    let nextCharCode = charCode + step;

    if (nextCharCode > lastCharCode) {
      nextCharCode = firstCharCode + (nextCharCode - lastCharCode) - 1;
    }
  ...
  return str.split('').reduce((acc, char) => {
    ...
    const charCode = isUppercase ? char.charCodeAt(0) : char.charCodeAt(0) - 32;

    if (charCode > lastCharCode || charCode < firstCharCode) {
      return acc + char;
    }

    const nextChar = String.fromCharCode(getNextCharCode(charCode));
    return (
      acc + (isUppercase ? nextChar.toUpperCase() : nextChar.toLowerCase())
    );
  }, '');
}
